...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\quanjogger'
$outProducts = Join-Path $folder 'insert-quanjogger-products.sql'
$outImages   = Join-Path $folder 'insert-quanjogger-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 29   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/quanjogger/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")

...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\hoodie'
$outProducts = Join-Path $folder 'insert-hoodie-products.sql'
$outImages   = Join-Path $folder 'insert-hoodie-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 23   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/hoodie/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")

...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\matkinh'
$outProducts = Join-Path $folder 'insert-matkinh-products.sql'
$outImages   = Join-Path $folder 'insert-matkinh-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 35   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/matkinh/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")

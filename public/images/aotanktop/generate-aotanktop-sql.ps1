...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\aotanktop'
$outProducts = Join-Path $folder 'insert-giaydep-products.sql'
$outImages   = Join-Path $folder 'insert-giaydep-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 24   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/aotanktop/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")

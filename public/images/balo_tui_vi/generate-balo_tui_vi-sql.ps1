...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\balo_tui_vi'
$outProducts = Join-Path $folder 'insert-balo_tui_vi-products.sql'
$outImages   = Join-Path $folder 'insert-balo_tui_vi-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 33   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/balo_tui_vi/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")

...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\setdo'
$outProducts = Join-Path $folder 'insert-setdo-products.sql'
$outImages   = Join-Path $folder 'insert-setdo-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 31   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/setdo/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")

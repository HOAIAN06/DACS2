...
# CONFIG
# ===============================
$folder      = 'C:\xampp\htdocs\hanzo-shop\public\images\quanshort'
$outProducts = Join-Path $folder 'insert-quanshort-products.sql'
$outImages   = Join-Path $folder 'insert-quanshort-images.sql'

# category_id của Áo Thun trong bảng categories
$catId = 27   # nếu khác thì sửa lại số này
...
foreach ($slug in $groups.Keys) {
    ...
    foreach ($img in $group) {
        $url    = "/images/aothun/$($img.File.Name)"
        $urlSql = $url.Replace("'", "''")
